...
class EarlyStopping(object):
    ...
    def __call__(self, current_score):
        if self.score is None:
            ...
            if np.abs(current_score - self.score) < self.thresh \
...
class Compressor(object):
    ...
    def _save_checkpoint(self, context):
        ...
        if context.epoch_id % 1 == 0 and self.checkpoint_path:
            checkpoint_path = os.path.join(self.checkpoint_path,'best_model')
    ...
    def run(self):
        ...
        count = 0
        best = 0
        early_stop = EarlyStopping(5, 0.0001)
        for epoch in range(start, self.epoch):
            ...
            try:
                ...
                if self.eval_epoch and epoch % self.eval_epoch == 0:
                    top1 = self._eval(context)[0]
                    signal = early_stop(top1)
                    print("!!!!!!!!!!!!11", signal)
                    if signal == True:
                        break
                    #if np.abs(top_1 - im_res) < 0.001 and epoch - pre_epoch == 1:
                    #    count += 1
                    #    im_res = top_1
                    #    pre_epoch = epoch
                    #else:
                ...
                #if count >=4:
                #    print("model has early stop in epoch ", epoch)
                #    break
                    if signal == 'best':
                        print("&&&&&&&&&&&&&&&&&&&")
                        self._save_checkpoint(context)
